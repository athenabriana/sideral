...
- Flakes are still technically experimental; enabling them globally puts silverfox ahead of upstream NixOS defaults.
...
**Chose**: Marker file at `/var/lib/silverfox/nix-setup-done`; unit runs while the marker is absent.

**Considered**: Fail loudly once; require manual `systemctl start` to retry.

**Why**:
- Matches the existing `silverfox-flatpak-install` and former `silverfox-mise-install` patterns.
