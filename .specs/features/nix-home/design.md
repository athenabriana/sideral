...
silverfox becomes a **three-layer stack**. Each layer has distinct lifecycle, ownership, and atomicity
guarantees.
...
|---|---|---|
| System oneshot + sentinel marker + idempotent retry | `system_files/etc/systemd/system/silverfox-flatpak-install.service` | `silverfox-nix-install.service` |
| User oneshot + per-user marker + `default.target.wants/` symlink | `system_files/usr/lib/systemd/user/silverfox-vscode-setup.service` | `silverfox-home-manager-setup.service` |
...
### 2. `silverfox-nix-install.service` — first-boot system oneshot
...
- **Location**: `system_files/etc/systemd/system/silverfox-nix-install.service` + symlink at
  `system_files/etc/systemd/system/multi-user.target.wants/silverfox-nix-install.service`.
- **Interfaces** (unit file):
  ...
  ConditionPathExists=!/var/lib/silverfox/nix-setup-done
  ...
  ExecStartPost=/usr/bin/bash -c 'mkdir -p /var/lib/silverfox && touch /var/lib/silverfox/nix-setup-done'
...
### 3. `silverfox-home-manager-setup.service` — first-login user oneshot
...
- **Location**: `system_files/usr/lib/systemd/user/silverfox-home-manager-setup.service` + symlink at
  `system_files/usr/lib/systemd/user/default.target.wants/silverfox-home-manager-setup.service`.
- **Interfaces** (unit file):
  ...
  ConditionPathExists=!%h/.cache/silverfox/home-manager-setup-done
  ...
  ExecStart=/usr/bin/bash -lc '\
      ...
      home-manager switch; \
      touch %h/.cache/silverfox/home-manager-setup-done'
  StandardOutput=journal
  StandardError=journal
...
- **Dependencies**: `silverfox-nix-install.service` must have completed on a prior boot (guarded by
...
- **Reuses**: Per-user marker + `WantedBy=default.target` symlink pattern from
  `silverfox-vscode-setup.service`. The existing `silverfox-mise-install.service` is removed entirely; this
...
| `system_files/usr/lib/systemd/user/silverfox-mise-install.service` | mise now via `home.packages` |
| `system_files/usr/lib/systemd/user/default.target.wants/silverfox-mise-install.service` | symlink orphan after unit removal |
...
|---|---|---|---|
| `/var/lib/silverfox/nix-setup-done` | system, per-host | nix installer ran successfully | `silverfox-nix-install.service` `ConditionPathExists!` guard |
| `~/.cache/silverfox/home-manager-setup-done` | user, per-user-per-host | home-manager bootstrap ran successfully | `silverfox-home-manager-setup.service` guard |
| `/var/lib/silverfox/flatpak-install-done` | system | (pre-existing, unchanged) | `silverfox-flatpak-install.service` |
| `~/.cache/silverfox/vscode-setup-done` | user | (pre-existing, unchanged) | `silverfox-vscode-setup.service` |
...
**Invariant 1: `silverfox-nix-install` runs only after `/var` is writable and network is up.**
- Enforced by `After=network-online.target ostree-remount.service`, `Wants=network-online.target`.

**Invariant 2: `silverfox-home-manager-setup` runs only after `/nix` exists and `nix` CLI works.**
- Enforced by `ConditionPathExists=/nix/var/nix/profiles/default/bin/nix` on the user unit.
- If the user logs in on the very boot where `silverfox-nix-install` is still running or failed, the
...
sequenceDiagram
    ...
    participant NixSvc as silverfox-nix-install
    participant User as User logs in
    participant HMSvc as silverfox-home-manager-setup
    ...
    NixSvc->>NixSvc: touch /var/lib/silverfox/nix-setup-done
    ...
    HMSvc->>HMSvc: touch ~/.cache/silverfox/home-manager-setup-done
```
...
| home-manager `switch` fails (eval error in `home.nix`) | Non-zero exit from `home-manager switch` | Marker absent; previous generation remains active (none on first switch); log in user journal | User inspects `journalctl --user -u silverfox-home-manager-setup`; typically a syntax error they just edited in |
| User deletes `~/.cache/silverfox/home-manager-setup-done` | N/A (user action) | Next login re-runs the whole bootstrap; home-manager is idempotent, so this is safe recovery | Safe "redo everything" escape hatch |
...
- `system_files/etc/systemd/system/silverfox-nix-install.service`
- `system_files/etc/systemd/system/multi-user.target.wants/silverfox-nix-install.service` (symlink → `../silverfox-nix-install.service`)
- `system_files/usr/lib/systemd/user/silverfox-home-manager-setup.service`
- `system_files/usr/lib/systemd/user/default.target.wants/silverfox-home-manager-setup.service` (symlink → `../silverfox-home-manager-setup.service`)
...
- `system_files/usr/lib/systemd/user/silverfox-mise-install.service`
...
- `system_files/etc/systemd/system/silverfox-flatpak-install.service` — parallel flow, unchanged.
- `system_files/usr/lib/systemd/user/silverfox-vscode-setup.service` — parallel flow, unchanged.
