...
## T02 — Ship `silverfox-nix-install.service` (system oneshot) + enablement symlink

- **What**: New unit at `system_files/etc/systemd/system/silverfox-nix-install.service` that runs
  `/usr/libexec/nix-installer install ostree --persistence /var/lib/nix --no-confirm`, then
  `restorecon -Rv /nix`, then touches `/var/lib/silverfox/nix-setup-done`. Guarded by
  `ConditionPathExists=!/var/lib/silverfox/nix-setup-done`. Symlink under
  `multi-user.target.wants/`.
- **Where**: `system_files/etc/systemd/system/silverfox-nix-install.service`,
  `system_files/etc/systemd/system/multi-user.target.wants/silverfox-nix-install.service` (relative
  symlink).
- **Depends on**: T01 (unit expects `/usr/libexec/nix-installer`).
- **Reuses**: Marker + `ConditionPathExists=!` pattern from `silverfox-flatpak-install.service`.
- **Done when**: Both files present. Symlink target is `../silverfox-nix-install.service`. Unit parses
...
## T04 — Ship `silverfox-home-manager-setup.service` (user oneshot) + enablement symlink

- **What**: New user unit at
  `system_files/usr/lib/systemd/user/silverfox-home-manager-setup.service` that: sources nix profile,
  ...
  `%h/.cache/silverfox/home-manager-setup-done`. Dual `ConditionPathExists`: `!marker` and
  `/nix/var/nix/profiles/default/bin/nix`. Symlink under `default.target.wants/`.
- **Where**: `system_files/usr/lib/systemd/user/silverfox-home-manager-setup.service`,
...
- **Reuses**: Per-user marker + `WantedBy=default.target` pattern from
  `silverfox-vscode-setup.service`.
...
## T06 — Remove `silverfox-mise-install.service` and its symlink `[P]`

- **What**: Delete `system_files/usr/lib/systemd/user/silverfox-mise-install.service` and
  `system_files/usr/lib/systemd/user/default.target.wants/silverfox-mise-install.service`.
- **Where**: repo.
- **Depends on**: T04 (new user unit supersedes this one).
- **Reuses**: —
- **Done when**: Neither file exists. `grep -r 'silverfox-mise-install' system_files/ build_files/`
  returns nothing.
- **Tests**: `just build` succeeds.
- **Gate**: Build.
...
- **What**: Add a section to `README.md` covering (a) the first-boot `silverfox-nix-install.service`
  behavior and journalctl location, (b) first-login `silverfox-home-manager-setup.service`, (c)
