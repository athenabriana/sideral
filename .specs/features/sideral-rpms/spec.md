...
| Browser packaging | Browser is **Helium via the community `helium` Flatpak remote** from `ShyVortex/helium-flatpak` (ostree archive-z2 served from GitHub Pages). The remote is registered + the app preinstalled at image build by `os/build.sh`; updates flow via standard `flatpak update`. The `imput/helium` COPR was tried twice and broke both times on the same `/opt` cpio conflict — see ATH-12 in `.specs/features/sideral/spec.md`. |
...
4. **ACR-23** — The current 8-entry manifest (Helium browser via the community `helium` Flatpak remote + 7 GNOME quality-of-life apps from flathub) ships in this package; future additions/removals are made by editing `packages/sideral-flatpaks/src/etc/flatpak-manifest` and the next image build picks it up automatically. Manifest format: `<remote> <ref>` only. The companion `/etc/sideral-flatpak-remotes` (3 entries: flathub, fedora, helium) ships in the same package and is read by both `os/build.sh` (image-build remote registration + preinstall) and `sideral-flatpak-install.service` (forward-compat self-heal).

**Test**: Fresh image with sideral-base installed → reboot → `flatpak list --app` shows 8 refs (including `net.imput.helium`); `flatpak remotes` shows the three curated remotes. `rpm-ostree override remove sideral-flatpaks` → next reboot → `/etc/flatpak-manifest` and `/etc/sideral-flatpak-remotes` absent, but already-installed flatpaks and registered remotes remain.
