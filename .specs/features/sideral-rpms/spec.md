...
| Browser packaging | Browser is **Zen Browser** (`app.zen_browser.zen` from Flathub). Preinstalled at image build by `os/build.sh`; updates via standard `flatpak update`. The `imput/helium` COPR was tried twice and broke both times on the same `/opt` cpio conflict; community Helium flatpak packagings (MarioGK/ShyVortex) didn't pan out either — see ATH-12 in `.specs/features/sideral/spec.md`. |
