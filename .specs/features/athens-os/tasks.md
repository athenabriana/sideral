...
### T002 [x] Rename image `fedora-athens` → `athens-os`
...
- **Done when:** `rg -l 'fedora-athens' .` returns nothing except possibly `.specs/` context.
- **Gate:** none (text-only).
- **Satisfies:** spec goal "Image `ghcr.io/<user>/athens-os:latest`".

### T003 [x] Rewrite `/etc/os-release` at build time
- **What:** Ensure the built image self-identifies as Athens OS. Done via `sed` in `build.sh` (folded into T007); this task only confirms the expected lines and documents the decision.
...
- **What:** Single script that:
  ...
  4. Rewrites `/etc/os-release`: `ID=athens-os`, `NAME="Athens OS"`, `PRETTY_NAME="Athens OS 43 (Silverblue)"`.
...
- **Done when:** `just build` completes; `bootc container lint` exits 0; `podman run --rm <img> cat /etc/os-release | grep -q 'ID=athens-os'`.
...
- **Where:**
  - `system_files/etc/flatpak-manifest` — one ref per line:
    ...
    ```
  - `system_files/etc/systemd/system/athens-flatpak-install.service` — `Type=oneshot`, `ConditionPathExists=!/var/lib/athens/flatpak-install-done`, reads the manifest, runs `flatpak install -y --noninteractive` per ref (skips already-installed), writes the sentinel on success, exits non-blocking on network failure.
  - Enable via `system_files/etc/systemd/system/multi-user.target.wants/athens-flatpak-install.service` symlink.
- **Depends on:** none.
...
- **What:** Confirm every file that already exists matches the spec; fix drift if any.
  - `system_files/etc/dconf/db/local.d/00-athens-focus` ✓
  - `system_files/etc/dconf/db/local.d/00-athens-gnome-shell` ✓
  - `system_files/etc/dconf/db/local.d/10-athens-keybinds` ✓
  - `system_files/etc/yum.repos.d/vscode.repo` ✓ (enabled=1 per decision)
  - `system_files/usr/lib/systemd/user/athens-mise-install.service` ✓ (installs mise + eagerly installs act/atuin/direnv)
  - `system_files/usr/lib/systemd/user/athens-vscode-setup.service` ✓ (installs 3 extensions + marker)
  - `system_files/usr/lib/systemd/user/default.target.wants/athens-mise-install.service` (symlink) ✓
  - `system_files/usr/lib/systemd/user/default.target.wants/athens-vscode-setup.service` (symlink) ✓
- **Depends on:** none.
...
- **Where:** `README.md`.
- **Depends on:** T001, T002.
- **Done when:** `rg -l 'hyprland|ags|astal|waybar|rofi|wlogout|kitty|bluefin-dx' README.md` returns empty; image name everywhere is `athens-os`.
- **Gate:** none (docs).
- **Satisfies:** spec "Zero reference to hyprland/bluefin in runtime artifacts".

### T014 [x] Update `.github/workflows/build.yml`
- **What:** Rename workflow + env to `athens-os`; rewrite `IMAGE_DESC` to GNOME/silverblue narrative; keep cosign + tagging logic.
- **Where:** `.github/workflows/build.yml`.
- **Depends on:** T002.
- **Done when:** `name: build-athens-os`, `IMAGE_NAME: athens-os`, `IMAGE_DESC` mentions GNOME + tiling-shell, no Hyprland/AGS/astal references.
