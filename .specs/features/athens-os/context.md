...
> - **Wave 1 (nix-home, NXH-01..40, ~2026-04 mid):** mise moved from RPM to nix; `athens-mise-install.service` removed; `act`, `atuin`, `direnv` dropped from mise toolchain or moved to home-manager; `/etc/skel/.bashrc` replaced by home-manager-generated `~/.bashrc`.
...
- ~~`system_files/usr/lib/systemd/user/athens-vscode-setup.service`~~ — service deleted; extensions now declared via `programs.vscode.extensions = with pkgs.vscode-extensions; [ ms-vscode-remote.remote-ssh, ms-vscode-remote.remote-containers ]`
...
- `athens-mise-install.service` **deleted**; `athens-home-manager-setup.service` materializes mise + the rest of home.nix on first user login.
...
### Athens-os specifics — current (post-cleanup)
...
- `/etc/dconf/db/local.d/{00-athens-focus, 00-athens-gnome-shell, 10-athens-keybinds}` — captured GNOME defaults
- `/etc/distrobox/distrobox.conf` — auto-mount `/nix`, `/var/lib/nix`, `/etc/nix` into every distrobox container ⊕ added 2026-04-23
- `/etc/flatpak-manifest` + `/etc/systemd/system/athens-flatpak-install.service` — 8-app auto-install
- `/etc/os-release` rewritten with `ID=athens-os`, `NAME="Athens OS"`
- `/etc/profile.d/athens-hm-status.sh` — first-shell bootstrap UX (poll home-manager-setup-done marker, source env when ready)
- `/etc/selinux/targeted/contexts/files/file_contexts.local` — `/nix` SELinux fcontext rules (mapping to `usr_t`/`bin_t`/`lib_t`)
- `/etc/systemd/system/athens-nix-install.service` — first-boot nix installer (multi-target.wants symlink)
- `/etc/systemd/system/athens-nix-relabel.{service,path}` — auto-restorecon on `/nix/store` mutations (multi-target.wants symlink for the .path)
...
- `/usr/lib/systemd/user/athens-home-manager-setup.service` + `default.target.wants/` symlink — first-login home-manager bootstrap
- ~~`/usr/lib/systemd/user/athens-vscode-setup.service`~~ — **deleted** 2026-04-23
- ~~`/usr/lib/systemd/user/athens-mise-install.service`~~ — **deleted** in nix-home migration
- `/usr/libexec/nix-installer` — pinned binary, run by `athens-nix-install.service`
