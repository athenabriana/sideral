...
silverfox's user-config layer was migrated to nix + home-manager via the `nix-home` feature (40 requirements, implemented locally, never VM-verified). Research surfaced three documented, still-unresolved frictions that hit Fedora atomic 42+ specifically:
...
silverblue-main:43 is in the impact zone for all three. Nix's unique value on atomic Fedora — per-user `nix profile install` for ad-hoc CLI tools — is not a workload silverfox exercises heavily, and the declarative-home-config benefit is recoverable without nix.

This feature retires `nix-home` before it ships and replaces it with a chezmoi-based dotfile workflow + RPM-layered CLI tools. chezmoi is a static Go binary in Fedora's repos with no daemon, no `/nix` store, no SELinux dance, and no composefs interaction. CLI tools that lived in `home.nix` (mise, starship, atuin, fzf, bat, eza, ripgrep, zoxide, gh, git-lfs, gcc/make/cmake, vscode) move back to RPM via Fedora's repos, `mise.jdx.dev/rpm`, or `packages.microsoft.com`. Shell-init wiring (the `eval "$(starship init bash)"` snippets that home-manager's `programs.X.enable` auto-emitted) is centralized in a single `/etc/profile.d/silverfox-cli-init.sh` shipped by `silverfox-shell-ux`, gated on `command -v` checks.

Result: same declarative-on-first-boot UX, fewer moving parts, no nix-shaped failure modes.

## Goals

- [ ] All nix-related artifacts removed: installer binary at `/usr/libexec/nix-installer`, `silverfox-nix-install.service`, `silverfox-nix-relabel.{path,service}`, `silverfox-home-manager-setup.service`, `/etc/distrobox/distrobox.conf` `/nix` mounts, `/etc/skel/.config/home-manager/home.nix`, and the obsolete `/etc/profile.d/silverfox-hm-status.sh` home-manager bootstrap waiter
- [ ] New `silverfox-cli-tools` meta sub-package with `Requires:` on 14 small Fedora-main / mise / Microsoft RPMs: chezmoi, mise, starship, atuin, fzf, bat, eza, ripgrep, zoxide, gh, git-lfs, gcc, make, cmake — plus `code`
...
- [ ] Shell-init wiring via `/etc/profile.d/silverfox-cli-init.sh` in `silverfox-shell-ux`
...
| Feature | Reason |
|---|---|
| Shipping a silverfox-flavored chezmoi source tree as default | User brings their own dotfiles repo; silverfox provides the bootstrap path, not the contents |
...
3. **CHM-03** — `silverfox-services` (per `os/packages/silverfox-services/silverfox-services.spec`) does not ship `silverfox-nix-install.service`, `silverfox-nix-relabel.path`, `silverfox-nix-relabel.service`, or `silverfox-home-manager-setup.service`. Their wants-symlinks under `multi-user.target.wants/` and `default.target.wants/` are also removed. The `.spec` is cleaned of any nix-specific `%post` / `%postun` scriptlets, `Requires:` lines, and inline comments referring to removed units — the .spec stops mentioning nix entirely.
4. **CHM-04** — `/etc/distrobox/distrobox.conf` (in `silverfox-base`) does not declare auto-mounts for `/nix`, `/var/lib/nix`, or `/etc/nix`. The bashrc snippet does not source `nix-daemon.sh`.
5. **CHM-05** — `silverfox-user` does not ship `/etc/skel/.config/home-manager/`. `/etc/skel/.config/home-manager/home.nix` is absent. The repo's top-level `home/` directory is removed.

**Test**: `find / -name '*nix-installer*'` returns nothing in the built image. `rpm -ql silverfox-services` lists no `nix-*` unit. `rpm -qf /etc/skel/.config/home-manager/home.nix` reports no owner.
...
**Acceptance**:
1. **CHM-06** — New sub-package `silverfox-cli-tools` (with spec at `os/packages/silverfox-cli-tools/silverfox-cli-tools.spec`). Meta-package, no files of its own. `Requires:`: `chezmoi`, `mise`, `starship`, `atuin`, `fzf`, `bat`, `eza`, `ripgrep`, `zoxide`, `gh`, `git-lfs`, `gcc`, `make`, `cmake`, `code`. Sub-package included in `os/packages/build-rpms.sh`.
2. **CHM-07** — `silverfox-base.spec` `Requires:` is extended with `silverfox-cli-tools` (matches the existing pattern for `silverfox-flatpaks`).
3. **CHM-08** — `os/packages/silverfox-base/src/etc/yum.repos.d/mise.repo` is shipped (mirrors `https://mise.jdx.dev/rpm/mise.repo`). `os/build.sh` registers it inline at build time (same persistent-repo pattern as `docker-ce.repo`) and runs `dnf5 install -y mise`. The repo file is `enabled=1` in the shipped image so `rpm-ostree upgrade` can pull mise updates.
4. **CHM-09** — `os/packages/silverfox-base/src/etc/yum.repos.d/vscode.repo` is shipped (mirrors `https://packages.microsoft.com/yumrepos/vscode/config.repo` with `gpgkey=https://packages.microsoft.com/keys/microsoft.asc` and `gpgcheck=1`). `os/build.sh` registers it inline at build time (same persistent-repo pattern as `mise.repo` / `docker-ce.repo`) and runs `dnf5 install -y code`. The repo file is `enabled=1` in the shipped image so `rpm-ostree upgrade` can pull VS Code updates between rebuilds.
...
6. **CHM-23** — Install order satisfies `silverfox-cli-tools.rpm`'s `Requires:` *before* the inline-rpmbuild RUN block runs. Concrete: a new `os/features/cli/packages.txt` lists the 13 Fedora-main RPMs (`chezmoi starship atuin fzf bat eza ripgrep zoxide gh git-lfs gcc make cmake`) and `os/build.sh` runs the existing per-feature dnf install loop against it, plus the inline `dnf5 install -y mise code` from CHM-08/09. All 15 Requires-targets are present in the image at the moment `rpm -Uvh /tmp/rpmbuild/RPMS/noarch/silverfox-cli-tools-*.rpm` runs in the inline-RPM RUN block. Same pattern as `bazaar` was previously satisfied (build.sh dnf-install, then rpm-build, then rpm -Uvh).

**Test**: `rpm -q silverfox-cli-tools chezmoi mise starship atuin fzf bat eza ripgrep zoxide gh git-lfs gcc make cmake code` reports all installed. `rpm -qa | grep ^silverfox-` lists 7 packages (silverfox-base + 6 sub-packages: services, flatpaks, dconf, shell-ux, signing, cli-tools — silverfox-user and silverfox-selinux dropped during chezmoi-home implementation). `rpm -V silverfox-cli-tools` finds no missing-dep failures.

---

### P1: Shell-init wiring shipped centrally ⭐ MVP

**Story**: A user opening a fresh bash shell on a silverfox image gets starship, atuin, zoxide, mise, and fzf integrations active without editing any dotfiles — same UX as nix-home's `programs.X.enable` magic, but via a single `/etc/profile.d/` script.

**Acceptance**:
1. **CHM-11** — `silverfox-shell-ux` (per `os/packages/silverfox-shell-ux/silverfox-shell-ux.spec`) ships `/etc/profile.d/silverfox-cli-init.sh`. Each integration line is guarded by `command -v <tool> >/dev/null 2>&1` so the file is robust against any single tool being uninstalled. The integrations:
...
3. **CHM-13** — `silverfox-shell-ux.spec` `%files` lists `/etc/profile.d/silverfox-cli-init.sh` and (per CHM-21) `/etc/profile.d/silverfox-onboarding.sh`. The obsolete `/etc/profile.d/silverfox-hm-status.sh` (home-manager bootstrap waiter) is **removed** from `%files` and from `packages/silverfox-shell-ux/src/etc/profile.d/`. `rpm -qf /etc/profile.d/silverfox-cli-init.sh` returns `silverfox-shell-ux`; `rpm -qf /etc/profile.d/silverfox-hm-status.sh` reports no owner.
...
**Acceptance**:
1. **CHM-14** — `README.md` contains a "Set up dotfiles" section documenting `chezmoi init --apply <your-repo>` as the first-login flow. Section explains that silverfox provides chezmoi but ships no default dotfiles repo.
...
**Acceptance**:
1. **CHM-21** — `silverfox-shell-ux` ships `/etc/profile.d/silverfox-onboarding.sh`. Gated on `[ ! -f "$HOME/.cache/silverfox/onboarding-shown" ]`. On first run: prints a single line — `Tip: run \`chezmoi init --apply <your-repo>\` if you have a dotfiles repo to apply.` — then `mkdir -p "$HOME/.cache/silverfox"` and `touch "$HOME/.cache/silverfox/onboarding-shown"`.
2. **CHM-22** — Subsequent shells silently skip (marker exists). The hint never repeats. The script is `0644` (sourced, not executed); errors writing to `$HOME/.cache/silverfox/` (e.g., read-only home) fail silently and do not abort shell startup (`mkdir -p ... 2>/dev/null || :`).

**Test**: First login on a fresh user → message appears, `~/.cache/silverfox/onboarding-shown` exists. Second shell — silent.
...
- **User without a chezmoi repo**: `chezmoi` is on `$PATH` but the user never runs `init`. `/etc/profile.d/silverfox-cli-init.sh` still wires shell integrations. Image works fully; dotfiles stay system defaults.
- **chezmoi source tree conflicts with `/etc/skel`-shipped files**: silverfox ships nothing user-facing under `/etc/skel/.bashrc`, `/etc/skel/.config/git/`, etc. Only file under `/etc/skel/` after this feature: nothing (was: `home.nix`, now removed).
- **User's chezmoi'd dotfiles duplicate `/etc/profile.d/silverfox-cli-init.sh`'s integrations**: harmless. `eval "$(starship init bash)"` twice is idempotent. Same for atuin/zoxide/mise/fzf.
...
This feature also restores the following requirements from `.specs/features/silverfox/spec.md` that `nix-home` had previously superseded:

| ID (silverfox) | Status before this feature | Restored to |
|---|---|---|
...
| ATH-24 (skel `.bashrc` activation) | Removed by nix-home | Replaced by `/etc/profile.d/silverfox-cli-init.sh` in silverfox-shell-ux (CHM-11). User's `~/.bashrc` is theirs alone |
| ATH-26 (silverfox-mise-install.service) | Removed by nix-home | **Not** restored. Mise installs at image-build time; no first-login service needed |
...
- [ ] `rpm -qa | grep ^silverfox-` lists 7 packages (silverfox-base + services + flatpaks + dconf + shell-ux + signing + cli-tools). silverfox-user and silverfox-selinux dropped during implementation.
- [ ] `bootc container lint` passes as the final RUN.
- [ ] Image size: VS Code adds ~115 MB and 14 small RPMs add ~30 MB; nix-installer (~25 MB) and /nix-related infra are removed. Net delta vs. silverfox-rpms Phase R image: estimated +100 to +120 MB. Acceptable trade for dropping nix.
- [ ] Cosign signing + push to ghcr.io still succeeds (no change to ACR-27..29).
- [ ] On a fresh VM rebased to the new image: opening a shell shows starship prompt, `z` works, Ctrl-R hits atuin, `mise --version` works, `chezmoi --version` works, `code --version` works. No `/nix` directory exists. `systemctl --user list-units` shows no `silverfox-home-manager-setup`. `systemctl list-units` shows no `silverfox-nix-install`.
- [ ] User runs `chezmoi init --apply https://github.com/<user>/dotfiles` and their declared dotfiles materialize.
- [ ] CI build remains under 12 minutes (was 6m24s for silverfox-rpms Phase R; this feature adds ~14 dnf installs but removes the nix-installer fetch).
